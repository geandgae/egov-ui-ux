  ...
  const createLnb = (menuData) => {
    ...
    if(lnb) {
      menuData.forEach((menu) => {
        // const liDepth1 = document.createElement("li");
        // const button = document.createElement("button");
        // button.type = "button";
        // button.className = "btn-menu-toggle";
        // button.innerHTML = menu.title;
        // liDepth1.appendChild(button);
        const liDepth1 = createElement("li", {},
          createElement("button", { type: "button", className: "btn-menu-toggle" }, menu.title)
        );
  
        if (menu.items) {
          // const ulDepth2 = document.createElement("ul");
          // ulDepth2.className = "depth2";
          ...
          menu.items.forEach((item) => {
            ...
            // const aDepth2 = document.createElement("a");
            // aDepth2.href = `${basePath}${menu.dir}/${item.file}.html`;
            // if (text.inside) {
            //   aDepth2.innerHTML = `${text.outside} <span class="sub">${text.inside}</span>`;
            // } else {
            //   aDepth2.innerHTML = text.outside;
            // }
            // liDepth2.appendChild(aDepth2);
            const aDepth2 = createElement("a", { href: `${basePath}${menu.dir}/${item.file}.html` },
              text.inside ? `${text.outside} <span class="sub">${text.inside}</span>` : text.outside
            );
            const liDepth2 = createElement("li", {}, aDepth2);
  
            if (item.subitems) {
              // const ulDepth3 = document.createElement("ul");
              // ulDepth3.className = "depth3";
              const ulDepth3 = createElement("ul", { className: "depth3" });
              item.subitems.forEach((subitem) => {
                const text = splitText(subitem.text);
                // const liDepth3 = document.createElement("li");
                // const aDepth3 = document.createElement("a");
                // aDepth3.href = `${basePath}${menu.dir}/${subitem.file}.html`;
                // if (text.inside) {
                //   aDepth3.innerHTML = `${text.outside} <span class="sub">${text.inside}</span>`;
                // } else {
                //   aDepth3.innerHTML = text.outside;
                // }
                // liDepth3.appendChild(aDepth3);
                const aDepth3 = createElement("a", { href: `${basePath}${menu.dir}/${subitem.file}.html` },
                  text.inside ? `${text.outside} <span class="sub">${text.inside}</span>` : text.outside
                );
